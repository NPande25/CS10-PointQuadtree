...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			// TODO: YOUR CODE HERE
			if (tree != null) tree.insert(new Dot(x, y)); // if there's a tree already, insert
			else tree = new PointQuadtree<>(new Dot(x, y), 0, 0, width, height); // if not, create new
		}
		else if (mode == 'q') {
...
//		tree.insert(new Dot(750, 550));
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(500, 350), 0, 0, width, height); // root A
		tree.insert(new Dot(130, 530)); // B
		tree.insert(new Dot(400, 500)); // C
		tree.insert(new Dot(100, 100)); // D
		tree.insert(new Dot(725, 300)); // E
		tree.insert(new Dot(200, 375)); // F
		tree.insert(new Dot(300, 425)); // G
		int bad = 0; // track total number of errors
		bad += testFind(100, 105, 10, 4, 2, 1); // rect for A [E] [D] [B]; circle A, D; find D
		bad += testFind(900, 700, 20, 1, 0, 0); // rect for A; circle for none; find none
		bad += testFind(300, 425, 10, 7, 5, 1); // rect for A [B [C [F [G]]] [D] [E]; circle for A, B, C, F, G; find G
	 ...
	 */
	public static void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		// Set the color for this level
		g.setColor(rainbow[level % rainbow.length]);
		// Draw this node's dot and lines through it
		// TODO: YOUR CODE HERE
		int r = dotRadius;
		g.fillOval((int) tree.getPoint().getX() - r, (int) tree.getPoint().getY() - r, 2 * r, 2 * r); // draw point
		g.drawLine(tree.getX1(), (int) tree.getPoint().getY(), tree.getX2(), (int) tree.getPoint().getY()); // draw horizontal line
		g.drawLine((int) tree.getPoint().getX(), tree.getY1(), (int) tree.getPoint().getX(), tree.getY2()); // draw vertical line
		...
		for (int i = 1; i <= 4; i++) {
			if (tree.hasChild(i)) {
				drawTree(g, tree.getChild(i), level + 1); // draw children
			}
		}
	}
